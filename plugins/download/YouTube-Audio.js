/*
/** This Audio Player code is originally created by
/** @Guru322 in Guru-Ai repository
/** Adapted for Moonlight by
/** @shizothetechie 
*/
...
const handler = async (m, { conn, command, text, args, usedPrefix }) => {
  if (!text) throw `give a text to search Example: *${usedPrefix + command}* pawan singh bhojpuri song`
  conn.SHIZOPLAY = conn.SHIZOPLAY ? conn.SHIZOPLAY : {}
  await conn.reply(m.chat, '⏳ *Searching...* Please wait while I find your music.', m)
  const result = await searchAndDownloadMusic(text)
  const infoText = `✦ ──『 *MOONLIGHT PLAYER* 』── ⚝ \n\n [ ⭐ Reply the number of the desired search result to get the Audio]. \n\n`
  ...
  conn.SHIZOPLAY[m.sender] = {
    ...
    timeout: setTimeout(() => {
      ...
      delete conn.SHIZOPLAY[m.sender]
    }, 150 * 1000),
  }
}

handler.before = async (m, { conn }) => {
  conn.SHIZOPLAY = conn.SHIZOPLAY || {}
  if (m.isBaileys || !(m.sender in conn.SHIZOPLAY)) return
  const { result, key, timeout } = conn.SHIZOPLAY[m.sender]
