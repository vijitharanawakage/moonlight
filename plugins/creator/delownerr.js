...
let handler = async (m, { conn, text }) => {
	...
	if (!owners.map(([number]) => number).map(v => v).includes(who)) return m.reply(`[ ! ] User is not in the real owner list.`)
	db.data.datas.rowner = owners.filter(([v]) => !v.includes(who))
	await conn.reply(m.chat, `Remove @${who} from the *real owner* list.`, m, { mentions: [who + '@s.whatsapp.net'] })
}

handler.help = ['delrealowner']
handler.tags = ['ownerr']
