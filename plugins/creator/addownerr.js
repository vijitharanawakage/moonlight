...
let handler = async (m, { conn, text, usedPrefix, command }) => {
	...
	if (!who) return m.reply(`Format : ${usedPrefix + command} nama | <tag / quote / type his number> `)
	let meh = await conn.onWhatsApp(who)
	if (meh.length == 0) return m.reply(`[!] Failed, @${(who.split('@')[0] || '')} not a WhatsApp user.`, null, { mentions: [who] })
	if (who == conn.user.jid) return m.reply(`[ ! ] Bot number automatically becomes real owner.`)
	...
	db.data.shizodb.rowner.push([who.split('@')[0], text[0], true])
	await conn.reply(m.chat, `Successfully made @${who.split('@')[0]} as the *real owner*.`, m, { mentions: [who] })
}
